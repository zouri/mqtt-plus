...
Item {
    ...
    function reloadStream(rows) {
        ...
        const start = Math.max(0, rows.length - root.maxEventRows)
    ...
    function trimStreamRows() {
        const overflow = eventStreamListModel.count - root.maxEventRows
        if (overflow > 0) {
    ...
    function loadOlderEvents() {
        ...
        const rows = root.appController.loadOlderCurrentEventRows()
        if (rows.length === 0) {
