...
Rectangle {
    ...
    ColumnLayout {
        ...
        ListView {
            ...
            clip: true
            spacing: 8
            currentIndex: control.sessions.currentSessionIndex
            model: control.sessions.sessions
            reuseItems: true
            ...
            delegate: Rectangle {
                ...
                readonly property bool selected: index === control.sessions.currentSessionIndex
                ...
                function showSessionContextMenu(globalPosition) {
                    const action = control.sessions.showSessionContextMenu(sessionDelegate.index, globalPosition);
                    if (action === "edit") {
                        control.sessionEditor.openForEdit(sessionDelegate.index);
                    } else if (action === "copy") {
                        control.sessions.duplicateSessionAt(sessionDelegate.index);
                    } else if (action === "delete") {
                        control.sessions.removeSessionAt(sessionDelegate.index);
                    }
                }

                Keys.onPressed: event => {
                    if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter || event.key === Qt.Key_Space) {
                        control.sessions.currentSessionIndex = sessionDelegate.index;
                        event.accepted = true;
